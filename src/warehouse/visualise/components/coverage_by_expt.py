...
def render(app: Dash, seq_data: object) -> html.Div:
    ...
    # Define colour map
    colour_map = {
        SeqDataSchema.N_UNMAPPED[1]: colours.GREY,
    ...
    @app.callback(
        Output(ids.SEQ_OUTPUT, "figure"), Input(ids.SEQ_OUTPUT_SCALE_BUTTON, "n_clicks")
    )
    ...
    df.sort_values(
        by="category",
        key=lambda col: col.map(SeqDataSchema.MAPPED_LIST.index),
        inplace=True,
    )
    ...
    return html.Div(
        children=[
            ...
            dcc.Graph(figure=fig, id=ids.SEQ_OUTPUT),
        ]
    )
